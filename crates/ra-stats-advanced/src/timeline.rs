...
impl Timeline {
    ...
    pub fn from_toml(input: &str) -> Result<Self, TimelineError> {
        let timeline: Self =
            toml::from_str(input).map_err(|e| TimelineError::ParseError(e.to_string()))?;
        timeline.validate()?;
        Ok(timeline)
    }
    ...
    pub fn validate(&self) -> Result<(), TimelineError> {
        ...
        for snap in &self.snapshots {
            if !seen_offsets.insert(snap.time_offset) {
                return Err(TimelineError::DuplicateOffset(snap.time_offset));
            }
        }
    ...
    pub fn feedback_in_range(&self, start: u64, end: u64) -> Vec<&ExecutionFeedback> {
        self.feedback
            .iter()
...
impl Snapshot {
    ...
    pub fn to_managed_stats(&self) -> HashMap<String, ManagedTableStats> {
        ...
        for table in &self.tables {
            ...
            let table_size = table
                .table_size_bytes
                .unwrap_or_else(|| table.row_count * avg_row_size as u64);
            ...
            let state = StatisticsState::new(StatisticsSource::ExactCount, table.row_count);
...
impl TimelinePlayer {
    ...
    pub fn current_time(&self) -> Option<u64> {
        self.current_snapshot().map(|s| s.time_offset)
    }

    /// Get the current snapshot, if positioned at one.
    pub fn current_snapshot(&self) -> Option<&Snapshot> {
        match self.position {
            PlaybackState::AtSnapshot(i) => self.timeline.snapshots.get(i),
            _ => None,
        }
    }

    /// Get managed stats for the current snapshot.
    pub fn current_managed_stats(&self) -> Option<HashMap<String, ManagedTableStats>> {
    ...
    pub fn step_backward(&mut self) -> PlaybackState {
        self.position = match self.position {
            PlaybackState::BeforeStart | PlaybackState::AtSnapshot(0) => PlaybackState::BeforeStart,
            PlaybackState::AtSnapshot(i) => PlaybackState::AtSnapshot(i - 1),
            PlaybackState::AfterEnd => {
    ...
    pub fn seek(&mut self, index: usize) -> Result<PlaybackState, TimelineError> {
    ...
    pub fn has_next(&self) -> bool {
        match self.position {
            ...
            PlaybackState::AtSnapshot(i) => i + 1 < self.timeline.snapshots.len(),
    ...
    pub fn row_count_delta(&self, table: &str, from_idx: usize, to_idx: usize) -> Option<i64> {
        let from_snap = self.timeline.snapshots.get(from_idx)?;
        let to_snap = self.timeline.snapshots.get(to_idx)?;

        let from_rows = from_snap.tables.iter().find(|t| t.name == table)?.row_count;
        let to_rows = to_snap.tables.iter().find(|t| t.name == table)?.row_count;
    ...
    pub fn compute_delta(&self, from_idx: usize, to_idx: usize) -> Option<crate::delta::DeltaSet> {
    ...
    pub fn cumulative_delta(&self, from_idx: usize) -> Option<crate::delta::DeltaSet> {
    ...
    pub fn last_analyze_snapshot_idx(&self) -> Option<usize> {
        ...
        for event in &self.timeline.events {
            if event.kind == EventKind::Analyze && event.time_offset <= current_time {
...
"#
    ...
    #[test]
    fn parse_minimal() {
        let tl = Timeline::from_toml(minimal_toml()).expect("parse minimal");
    ...
    #[test]
    fn parse_full() {
        let tl = Timeline::from_toml(full_toml()).expect("parse full");
    ...
    fn parse_snapshot_labels() {
        ...
        assert_eq!(tl.snapshots[0].label, Some("initial".to_string()));
        assert_eq!(tl.snapshots[1].label, Some("after inserts".to_string()));
    }
    ...
    fn step_backward_at_before_start_stays() {
        let tl = Timeline::from_toml(minimal_toml()).expect("parse");
        let mut player = TimelinePlayer::new(tl).expect("create");
        assert_eq!(player.step_backward(), PlaybackState::BeforeStart);
    }
    ...
    fn step_backward_middle() {
        ...
        assert_eq!(player.step_backward(), PlaybackState::AtSnapshot(1));
    }
    ...
    fn timeline_serialize_roundtrip() {
        ...
        let tl2 = Timeline::from_toml(&serialized).expect("re-parse");
        assert_eq!(tl, tl2);
    }
    ...
    fn timeline_json_roundtrip() {
        ...
        let tl2: Timeline = serde_json::from_str(&json).expect("from json");
        assert_eq!(tl, tl2);
    }
    ...
    fn event_kind_insert() {
        let input = r#""insert""#;
        let kind: EventKind = serde_json::from_str(input).expect("parse");
...
"#;
    ...
    fn playback_state_equality() {
        ...
        assert_eq!(PlaybackState::AtSnapshot(1), PlaybackState::AtSnapshot(1));
        assert_ne!(PlaybackState::AtSnapshot(0), PlaybackState::AtSnapshot(1));
    ...
    fn error_display_out_of_bounds() {
        let err = TimelineError::SnapshotOutOfBounds { index: 5, total: 3 };
        let msg = format!("{err}");
    ...
    fn example_join_reordering_cascade_feedback_stale_period() {
        ...
        let stale_fb: Vec<_> = tl.feedback.iter().filter(|f| f.q_error() > 1.5).collect();
        assert!(
            !stale_fb.is_empty(),
    ...
    fn example_join_reordering_cascade_events() {
        ...
        let reopt_count = tl
            ...
            .count();
        assert!(reopt_count >= 3, "should have multiple reoptimize events");
    }
    ...
    fn example_index_vs_seqscan_plan_transitions() {
        ...
        let has_index = tl
            .feedback
            .iter()
            .any(|f| f.operator.as_ref().is_some_and(|o| o.contains("IndexScan")));
        let has_seq = tl
            .feedback
            .iter()
            .any(|f| f.operator.as_ref().is_some_and(|o| o.contains("SeqScan")));
    ...
    fn example_index_vs_seqscan_accuracy_recovery() {
        let tl = load_example("index-vs-seqscan.toml");
        let last = tl.feedback.last().expect("last");
        assert!(last.q_error() < 1.1, "final feedback should be accurate");
    }
    ...
    fn example_aggregation_strategy_parses() {
        let tl = load_example("aggregation-strategy-evolution.toml");
        assert_eq!(tl.metadata.name, "aggregation-strategy-evolution");
    ...
    fn example_aggregation_archive_shrinks_table() {
        ...
        let delta = player.row_count_delta("sensor_readings", 7, 8);
    ...
    fn example_partition_pruning_parses() {
        let tl = load_example("partition-pruning-effectiveness.toml");
        assert_eq!(tl.metadata.name, "partition-pruning-effectiveness");
        assert_eq!(tl.snapshot_count(), 8);
    }
    ...
    fn example_partition_pruning_has_partitions() {
        ...
        assert!(names.iter().any(|n| n.starts_with("event_log_2025_10")));
    ...
    fn example_partition_pruning_stale_period() {
        ...
        let stale_fb: Vec<_> = tl.feedback.iter().filter(|f| f.q_error() > 3.0).collect();
        assert!(
            !stale_fb.is_empty(),
    ...
    fn example_partition_pruning_events() {
        let tl = load_example("partition-pruning-effectiveness.toml");
        assert!(tl.event_count() >= 10);
        let has_schema_change = tl.events.iter().any(|e| e.kind == EventKind::SchemaChange);
        assert!(
            has_schema_change,
    ...
    fn all_examples_have_valid_event_kinds() {
        ...
        for file in &files {
            ...
            for event in &tl.events {
                ...
                assert!(!event.table.is_empty(), "{file}: event has empty table");
            }
        }
    }
    ...
    fn all_examples_feedback_q_error_at_least_one() {
        ...
        for file in &files {
            ...
            for fb in &tl.feedback {
                assert!(fb.q_error() >= 1.0, "{file}: Q-error should be >= 1.0");
            }
        }
    }
    ...
    fn all_examples_snapshots_sorted() {
        ...
        for file in &files {
            ...
            for i in 1..tl.snapshots.len() {
                assert!(
                    tl.snapshots[i].time_offset > tl.snapshots[i - 1].time_offset,
    ...
    fn example_mixed_workload_events() {
        let tl = load_example("mixed-workload.toml");
        assert!(tl.event_count() >= 10);
        let has_insert = tl.events.iter().any(|e| e.kind == EventKind::Insert);
        let has_update = tl.events.iter().any(|e| e.kind == EventKind::Update);
        let has_delete = tl.events.iter().any(|e| e.kind == EventKind::Delete);
        assert!(has_insert);
        assert!(has_update);
        assert!(has_delete);
    }
