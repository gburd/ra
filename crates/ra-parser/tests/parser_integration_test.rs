...
use ra_parser::{
    ParseError, parse_metadata, parse_rule_file,
...
fn parse_valid_simple_rule_fixture() {
    let source =
        helpers::load_fixture("valid-simple-rule.rra");
    let rule = parse_rule_file(&source)
        .expect("valid-simple-rule.rra should parse");
...
fn parse_valid_complex_rule_fixture() {
    let source =
        helpers::load_fixture("valid-complex-rule.rra");
    let rule = parse_rule_file(&source)
        .expect("valid-complex-rule.rra should parse");
...
fn parse_invalid_missing_fields_fixture() {
    let source =
        helpers::load_fixture("invalid-missing-fields.rra");
...
fn parse_invalid_bad_yaml_fixture() {
    let source =
        helpers::load_fixture("invalid-bad-yaml.rra");
    let err = parse_rule_file(&source)
        .expect_err("invalid-bad-yaml.rra should fail");
    assert!(
...
fn parse_invalid_bad_category_fixture() {
    let source =
        helpers::load_fixture("invalid-bad-category.rra");
    let result = parse_rule_file(&source);
    assert!(
        result.is_err(),
...
fn missing_frontmatter_error_is_descriptive() {
    let err = parse_rule_file("# No frontmatter here")
...
fn invalid_yaml_error_includes_line_info() {
    ...
    match err {
        ParseError::InvalidYaml { line, .. } => {
            assert!(
                line > 0,
                "line number should be positive, got: {line}"
            );
        }
        other => {
...
fn metadata_round_trip_for_all_valid_fixtures() {
    for name in &[
        ...
        let full = parse_rule_file(&source)
            .unwrap_or_else(|e| {
                panic!("{name} full parse failed: {e}")
            });

        let meta_only = parse_metadata(&source)
            .unwrap_or_else(|e| {
                panic!("{name} metadata parse failed: {e}")
            });
