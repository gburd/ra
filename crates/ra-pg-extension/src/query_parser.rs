...
use ra_core::algebra::{
    AggregateExpr, AggregateFunction, CycleDetection, JoinType,
    NullOrdering, ProjectionColumn, RelExpr, SortDirection, SortKey,
};
...
/// utility statements, window functions, DML).
///
/// # Safety
///
...
unsafe fn build_cte_definitions(
    ...
    for i in 0..len {
        let node = pg_sys::list_nth(cte_list, i)
        ...
        let cte_def =
            match parse_with_depth(cte_query, depth + 1)? {
                Some(def) => def,
...
unsafe fn build_cycle_detection(
    ...
    if !col_list.is_null() {
        ...
        for i in 0..len {
            let val = pg_sys::list_nth(col_list, i)
    ...
    let cycle_mark_column =
        resolve_cte_name(cc.cycle_mark_column);
    let path_column =
        resolve_cte_name(cc.cycle_path_column);
...
unsafe fn build_rte_scan(
    ...
    if rtekind == pg_sys::RTEKind::RTE_CTE {
        ...
        let name = CStr::from_ptr(ctename)
            .to_string_lossy()
            .into_owned();
        let alias = resolve_alias(rte);
        return Ok(Some(RelExpr::Scan {
            table: name,
            alias,
        }));
    }
...
mod tests {
    ...
    fn wrap_recursive_cte() {
        ...
        let result = wrap_with_ctes(body, defs);
        assert!(
            matches!(result, RelExpr::RecursiveCTE { .. })
        );
    }
    ...
    fn resolve_cte_name_null_returns_empty() {
        let result = unsafe {
            resolve_cte_name(std::ptr::null_mut())
        };
