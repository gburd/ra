...
fn demo_exact_match() {
    ...
    println!("  Individual NDVs: city={}, state={}", individual_ndvs[0], individual_ndvs[1]);
    println!("  Independence assumption: {} distinct pairs", independence_estimate);
    println!("  Multi-column statistics: {} distinct pairs", multi_estimate);
    println!("  Actual distinct pairs:   100");
    println!("  Q-error (independence):  {:.1}x", independence_estimate as f64 / 100.0);
    println!("  Q-error (multi-column):  {:.1}x\n", multi_estimate as f64 / 100.0);
}
...
fn demo_prefix_match() {
    ...
    println!("  Independence assumption: {} pairs", independence_estimate);
    println!("  Multi-column statistics: {} pairs", multi_estimate);
    println!("  Improvement: {:.1}x better\n", independence_estimate as f64 / multi_estimate as f64);
}
...
fn demo_superset_match() {
    ...
    let columns = vec!["product".to_string(), "category".to_string(), "brand".to_string()];
    ...
    println!("  Independence assumption: {} combinations", independence_estimate);
    println!("  Blended estimate:        {} combinations", multi_estimate);
...
fn demo_with_histogram() {
    ...
    let hist = MultiDimHistogram::new(
        ...
        vec![
            ...
            vec![0.0, 30000.0, 60000.0, 100000.0],
        ],
        vec![
...
fn demo_configuration_presets() {
    ...
    for (name, config) in configs {
        println!("  {:<12} enabled={:<5} max_cols={} min_corr={:.2} min_improve={:.1}",
            name,
            config.enabled,
...
fn demo_q_error_comparison() {
    ...
    println!("  {:<30} {:<10} {:<10} {:<10} {:<12} {:<12}", "Scenario", "Indep", "Multi", "Actual", "Q-Err Indep", "Q-Err Multi");
    println!("  {:-<30} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}", "", "", "", "", "", "");

    for (name, ndv1, ndv2, actual, _corr) in scenarios {
        let mut estimator = MultiColumnEstimator::with_defaults();
        ...
        let q_err_indep = (independence as f64 / actual as f64).max(actual as f64 / independence as f64);
        let q_err_multi = (multi as f64 / actual as f64).max(actual as f64 / multi as f64);

        println!("  {:<30} {:<10} {:<10} {:<10} {:<12.1}x {:<12.1}x",
            name, independence, multi, actual, q_err_indep, q_err_multi);
    }
