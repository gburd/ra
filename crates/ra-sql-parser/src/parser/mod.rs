...
impl<'a> Parser<'a> {
    ...
    pub fn parse_infix(&mut self, expr: Expr, precedence: u8) -> Result<Expr, ParserError> {
        ...
        let regular_binary_operator = match &mut tok.token {
            ...
            Token::AtEquals => Some(BinaryOperator::AtEquals),
            Token::AtGreaterEquals => Some(BinaryOperator::AtGreaterEquals),
            Token::AtLessEquals => Some(BinaryOperator::AtLessEquals),
...
mod tests {
    ...
    fn test_tokenizer_error_loc() {
        ...
        assert_eq!(
            ast,
            Err(ParserError::TokenizerError(
                "Unterminated string literal at 1:5".to_string()
            ))
        );
    }
    ...
    fn test_parser_error_loc() {
        ...
        assert_eq!(
            ast,
            Err(ParserError::ParserError(
                "Expected: [NOT] NULL or TRUE|FALSE or [NOT] DISTINCT FROM after IS, found: a at 1:16"
                    .to_string()
            ))
        );
    }
