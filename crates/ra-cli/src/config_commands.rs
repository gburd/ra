...
pub fn cmd_config_list(quiet: bool) -> Result<()> {
    ...
    let max_key_len = RaConfig::keys().iter().map(|k| k.len()).max().unwrap_or(0);
...
pub fn cmd_config_set(key: &str, value: &str, quiet: bool) -> Result<()> {
    let loader = ConfigLoader::new();
    let mut config = loader.load().context("loading configuration")?;

    config.set(key, value).map_err(|e| anyhow::anyhow!("{e}"))?;
...
pub fn cmd_config_edit() -> Result<()> {
    ...
    match loader.load() {
        Ok(_) => {
            eprintln!("{}", "Configuration saved and validated.".green());
        }
        Err(e) => {
            eprintln!("{}: {e}", "Warning: config has errors".yellow().bold());
        }
    }
...
pub fn cmd_config_reset(quiet: bool) -> Result<()> {
    ...
    if !quiet {
        eprintln!("{}", "Configuration reset to defaults.".green().bold());
    }
