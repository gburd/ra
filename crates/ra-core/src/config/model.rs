...
impl Default for SqlConfig {
    fn default() -> Self {
        Self {
            ...
            dialect: ConfigSqlDialect::default(),
...
/// SQL dialect for parser features (configuration layer).
#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum ConfigSqlDialect {
    /// ANSI SQL standard.
    #[default]
    Ansi,
...
impl fmt::Display for ConfigSqlDialect {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl ParseFromStr for ConfigSqlDialect {
    fn parse_from_str(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
...
mod tests {
    ...
    fn sql_dialect_parse() {
        assert_eq!(
            ConfigSqlDialect::parse_from_str("pg"),
            Some(ConfigSqlDialect::Postgresql)
        );
        assert_eq!(
            ConfigSqlDialect::parse_from_str("mssql"),
            Some(ConfigSqlDialect::Sqlserver)
        );
        assert!(ConfigSqlDialect::parse_from_str("xyz").is_none());
    }
