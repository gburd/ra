...
use ra_engine::executors::lateral_join::{
    LateralJoinExecutor, LateralRhs,
};
use ra_engine::executors::table_function::TableFunctionExecutor;
use ra_engine::executors::unnest::{
    MultiUnnestExecutor, UnnestExecutor,
};
use ra_engine::Row;
    ...
    for size in [10, 100, 1000] {
        group.bench_function(
            ...
            |b| {
                ...
                let executor =
                    UnnestExecutor::new(expr, None, false);
                b.iter(|| {
                    black_box(executor.execute(None))
...
fn bench_unnest_with_ordinality(c: &mut Criterion) {
    let mut group =
        c.benchmark_group("unnest_with_ordinality");
...
fn bench_unnest_vs_generate_series(c: &mut Criterion) {
    let mut group =
        c.benchmark_group("unnest_vs_generate_series");
...
fn bench_multi_unnest(c: &mut Criterion) {
    ...
    for num_arrays in [2, 5, 10] {
        group.bench_function(
            format!("{num_arrays}_arrays_100"),
            |b| {
                let exprs: Vec<Expr> = (0..num_arrays)
                    .map(|_| make_int_array(100))
                    .collect();
                let aliases: Vec<Option<String>> =
                    (0..num_arrays).map(|_| None).collect();
        ...
        group.bench_function(
            ...
            |b| {
                ...
                let unnest = UnnestExecutor::new(
                    Expr::Column(
                        ra_core::expr::ColumnRef::new("arr"),
                    ),
                    None,
                    false,
                );
                let executor = LateralJoinExecutor::new(
                    LateralRhs::Unnest(unnest),
                    false,
                );

                b.iter(|| {
                    black_box(executor.execute(&left_rows))
                        .expect("should succeed");
                });
            },
        );
...
fn bench_json_array_elements(c: &mut Criterion) {
    let mut group =
        c.benchmark_group("json_array_elements");

    for size in [10, 100, 1000] {
        group.bench_function(
            format!("elements_{size}"),
            |b| {
