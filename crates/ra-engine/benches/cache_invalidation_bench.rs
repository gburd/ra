...
use ra_engine::differential::{
    ChangeSource, PlanDependencies, StatisticsChange,
};
...
fn make_table_plans(
    n: usize,
) -> Vec<(QueryFingerprint, RelExpr, PlanDependencies)> {
    (0..n)
        .map(|i| {
            ...
            let deps = PlanDependencies {
                table_cardinalities: [(table, 1000.0)]
                    .into_iter()
                    .collect(),
                indexes: HashSet::new(),
    ...
    for &num_plans in &[10, 100, 1000] {
        ...
        let provider = TestProvider {
            tables: plans
                .iter()
                .flat_map(|(_, _, deps)| {
                    deps.table_cardinalities.keys().map(|t| {
                        (t.clone(), Statistics::new(1000.0))
                    })
                })
                .collect(),
        };
...
fn bench_differential_invalidation(c: &mut Criterion) {
    let mut group =
        c.benchmark_group("invalidation_differential");
    ...
    for &num_entries in &[10, 100, 1000] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let result =
                        cache.lookup(black_box(&lookup_fp));
                    black_box(result);
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, &n| {
                b.iter(|| {
                    ...
                    let new =
                        Statistics::new(100_000.0);
                    let changes = opt.detect_changes(
                        "table_0", &old, &new,
                    );
