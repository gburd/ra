    ...
    pub fn best_match(&self, query: &RelExpr) -> Option<MvMatch> {
        ...
        matches.sort_by(|a, b| {
            let type_ord = match (&a.match_type, &b.match_type) {
                (MatchType::Exact, MatchType::Exact) => std::cmp::Ordering::Equal,
...
#[must_use]
pub fn match_query_with_mv(query: &RelExpr, mv: &MaterializedViewInfo) -> Option<MvMatch> {
    ...
    match pred_relation {
        PredicateRelation::Equivalent if agg_compatible => Some(MvMatch {
        ...
        PredicateRelation::QueryStronger if agg_compatible => {
            let compensation = compute_compensation(&query_preds, &mv_preds);
            Some(MvMatch {
                mv_name: mv.name.clone(),
        ...
        PredicateRelation::ViewStronger if agg_compatible => Some(MvMatch {
            mv_name: mv.name.clone(),
...
/// using.
#[must_use]
pub fn view_benefit(original_cost: f64, mv_scan_cost: f64, compensation_cost: f64) -> f64 {
    original_cost - (mv_scan_cost + compensation_cost)
}

// ── Internal helpers ─────────────────────────────────────────────

fn is_structurally_compatible(query: &RelExpr, mv_def: &RelExpr) -> bool {
...
fn collect_tables_inner(expr: &RelExpr, out: &mut HashSet<String>) {
    match expr {
        RelExpr::Scan { table, .. } => {
...
fn extract_predicates_inner(expr: &RelExpr, out: &mut Vec<Expr>) {
    match expr {
...
fn compare_predicates(query_preds: &[Expr], mv_preds: &[Expr]) -> PredicateRelation {
    let query_set: HashSet<String> = query_preds.iter().map(|p| format!("{p:?}")).collect();
    let mv_set: HashSet<String> = mv_preds.iter().map(|p| format!("{p:?}")).collect();
...
fn compute_compensation(query_preds: &[Expr], mv_preds: &[Expr]) -> Option<Expr> {
    let mv_set: HashSet<String> = mv_preds.iter().map(|p| format!("{p:?}")).collect();
...
fn aggregations_compatible(query: &RelExpr, mv_def: &RelExpr) -> bool {
    ...
    match (query_agg, mv_agg) {
        ...
        (Some((q_groups, q_aggs)), Some((m_groups, m_aggs))) => {
            ...
            for (qa, ma) in q_aggs.iter().zip(m_aggs.iter()) {
                if qa.function != ma.function || qa.distinct != ma.distinct {
                    return false;
                }
            }
            true
...
fn find_aggregate(expr: &RelExpr) -> Option<(&[Expr], &[ra_core::algebra::AggregateExpr])> {
    match expr {
        RelExpr::Aggregate {
            group_by,
            aggregates,
...
mod tests {
    ...
    fn make_order_summary_query(extra_pred: Option<Expr>) -> RelExpr {
    ...
    fn make_mv_info(extra_pred: Option<Expr>, row_count: u64) -> MaterializedViewInfo {
    ...
    fn catalog_best_match_prefers_exact() {
        ...
        catalog.add_view(MaterializedViewInfo {
            ...
            definition: make_order_summary_query(Some(gt(col("o_totalprice"), int(0)))),
            base_tables: vec!["customer".to_string(), "orders".to_string()],
            row_count: 100,
            is_incremental: false,
        });
    ...
    fn compensation_computed_for_extra_predicates() {
        let query_preds = vec![eq(col("a"), int(1)), gt(col("b"), int(2))];
