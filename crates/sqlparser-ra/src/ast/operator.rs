...
pub enum BinaryOperator {
    ...
    AtAt,
    /// The `@=` operator (DocumentDB/BSON exact match).
    AtEquals,
    /// The `@>=` operator (DocumentDB/BSON greater-than-or-equal).
    AtGreaterEquals,
    /// The `@<=` operator (DocumentDB/BSON less-than-or-equal).
    AtLessEquals,
    /// The `@>` operator.
    ///
    /// On PostgreSQL, this is used for JSON and text searches.
...
impl fmt::Display for BinaryOperator {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            BinaryOperator::AtEquals => f.write_str("@="),
            BinaryOperator::AtGreaterEquals => f.write_str("@>="),
            BinaryOperator::AtLessEquals => f.write_str("@<="),
