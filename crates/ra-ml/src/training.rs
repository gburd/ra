...
use crate::estimator::{
    CardinalityEstimator, SimpleStatsProvider, q_error,
};
use crate::features::{FeatureSchema, log_scale};
...
impl EvaluationResult {
    ...
    pub fn mean_q_error(&self) -> f64 {
        ...
        self.q_errors.iter().sum::<f64>()
            / self.q_errors.len() as f64
    }
}
...
mod tests {
    ...
    fn dataset_add_and_serialize() {
        ...
        let restored = TrainingDataset::from_json(&json)
            .expect("deserialize");
    ...
    fn evaluate_heuristic() {
        ...
        let result =
            evaluate_estimator(&est, &queries, &provider);
        assert_eq!(result.q_errors.len(), 2);
        assert!(
            (result.q_errors[0] - 1.0).abs() < f64::EPSILON
        );
    ...
    fn evaluation_result_stats() {
        ...
        assert!(
            (result.median_q_error() - 3.0).abs()
                < f64::EPSILON
        );
        assert!(
            (result.mean_q_error() - 3.0).abs() < f64::EPSILON
        );
    }
    ...
    fn evaluation_result_empty() {
        ...
        assert!(
            (result.median_q_error() - 0.0).abs()
                < f64::EPSILON
        );
        assert!(
            (result.mean_q_error() - 0.0).abs() < f64::EPSILON
        );
    }
