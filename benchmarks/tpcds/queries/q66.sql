-- TPC-DS Query 66
-- Revenue across shipping modes for web and catalog
SELECT
  w_warehouse_name,
  w_warehouse_sq_ft,
  w_city,
  ...
  SUM(jan_sales + feb_sales + mar_sales + apr_sales + may_sales + jun_sales +
      jul_sales + aug_sales + sep_sales + oct_sales + nov_sales + dec_sales) AS total_sales,
...
FROM (
  SELECT
    ...
    w_state,
    w_country,
    CONCAT(sm_type, ',', sm_carrier) AS ship_carriers,
    d_year,
  ...
  GROUP BY w_warehouse_name, w_warehouse_sq_ft, w_city, w_county, w_state, w_country,
           CONCAT(sm_type, ',', sm_carrier), d_year
