-- TPC-DS Query 83: Revenue for items returned in specific weeks
-- Compare revenue from store returns, catalog returns, and web returns
-- for items returned in specific weeks
WITH sr_items AS (
    SELECT
...
SELECT
    sr_items.item_id,
    sr_item_qty,
    sr_item_qty / (sr_item_qty + cr_item_qty + wr_item_qty)::NUMERIC / 3.0
        * 100 AS sr_dev,
    cr_item_qty,
    cr_item_qty / (sr_item_qty + cr_item_qty + wr_item_qty)::NUMERIC / 3.0
        * 100 AS cr_dev,
    wr_item_qty,
    wr_item_qty / (sr_item_qty + cr_item_qty + wr_item_qty)::NUMERIC / 3.0
        * 100 AS wr_dev,
