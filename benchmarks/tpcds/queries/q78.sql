-- TPC-DS Query 78: Report annual sales for customers from all three channels
-- Find customers who bought from all three channels and report annual sales
WITH ws AS (
    SELECT
        d_year AS ws_year,
        ws_item_sk,
...
SELECT
    ...
    ss.ss_item_sk,
    ss.ss_customer_sk,
    ROUND(ss_qty / (COALESCE(ws_qty, 0) + COALESCE(cs_qty, 0))::NUMERIC, 2)
        AS ratio,
    ss_qty AS store_qty,
...
FROM ss
LEFT JOIN ws ON ws.ws_year = ss.ss_year
    AND ws.ws_item_sk = ss.ss_item_sk
    AND ws.ws_customer_sk = ss.ss_customer_sk
LEFT JOIN cs ON cs.cs_year = ss.ss_year
