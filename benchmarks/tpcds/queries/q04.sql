-- TPC-DS Query 4
-- Find customers who spend more money via catalog than in stores in certain
...
WITH year_total AS (
    SELECT
        ...
        SUM(((ss_ext_list_price - ss_ext_wholesale_cost - ss_ext_discount_amt)
             + ss_ext_sales_price) / 2) AS year_total_amt,
        's' AS sale_type
    FROM customer
    ...
    SELECT
        ...
        SUM(((cs_ext_list_price - cs_ext_wholesale_cost - cs_ext_discount_amt)
             + cs_ext_sales_price) / 2) AS year_total_amt,
        'c' AS sale_type
    FROM customer
    ...
    SELECT
        ...
        SUM(((ws_ext_list_price - ws_ext_wholesale_cost - ws_ext_discount_amt)
             + ws_ext_sales_price) / 2) AS year_total_amt,
        'w' AS sale_type
    FROM customer
