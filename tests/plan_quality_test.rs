...
fn filter_on_nullable_side_not_pushed_below_left_join() {
    let plan =
        optimize("SELECT * FROM t1 LEFT JOIN t2 ON t1.id = t2.id WHERE t2.status = 'active'");
...
fn join_commutativity_produces_valid_plan() {
    let plan = optimize("SELECT * FROM small_table s JOIN large_table l ON s.id = l.sid");
...
fn redundant_filter_elimination() {
    let plan = optimize("SELECT * FROM t WHERE x = 1 AND x = 1");
    let debug = format!("{plan:?}");
    ...
    assert!(
        count <= 4,
        "redundant filter not eliminated: found {count} 'x' in {debug}"
    );
}
...
fn filter_before_aggregate_is_valid() {
    let plan = optimize("SELECT dept, COUNT(*) FROM emp WHERE salary > 50000 GROUP BY dept");
    let debug = format!("{plan:?}");
    assert!(debug.contains("salary"), "salary filter missing");
    assert!(
        debug.contains("Aggregate") || debug.contains("group_by"),
...
fn limit_not_pushed_below_join() {
    let plan = optimize("SELECT * FROM t1 JOIN t2 ON t1.id = t2.id LIMIT 10");
    let debug = format!("{plan:?}");
    assert!(
        debug.contains("Limit") || debug.contains("limit") || debug.contains("count: 10"),
        "LIMIT missing from plan: {debug}"
    );
    // LIMIT should be above the join
    assert!(
        debug.contains("Join") || debug.contains("join"),
...
fn window_function_preserved() {
    ...
    let debug = format!("{plan:?}");
    assert!(
        debug.contains("Window") || debug.contains("window"),
...
fn compound_filter_pushdown_through_join() {
    let plan = optimize("SELECT * FROM t1 JOIN t2 ON t1.id = t2.id WHERE t1.a = 1 AND t2.b = 2");
    let debug = format!("{plan:?}");
    ...
    let has_compound_above_join = debug.contains("And")
        && debug.find("And").unwrap() < debug.find("Join").unwrap_or(usize::MAX);
    assert!(
