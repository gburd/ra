# Rule: Calcite FilterTableScanRule

**Category:** logical/predicate-pushdown
**File:** `rules/logical/predicate-pushdown/filter-table-scan.rra`

## Metadata

- **ID:** `calcite-filter-table-scan`
- **Version:** "1.0.0"
...
```yaml
  - type: "pattern"
    ...
    description: "Filter above a table scan"
  - type: "predicate"
    condition: "is_deterministic(?pred)"
    description: "Predicate must be deterministic"
  - type: "predicate"
    condition: "references_only(?pred, columns(?table))"
    description: "Predicate must reference only columns from the table"
```
