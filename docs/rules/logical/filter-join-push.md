...
**File:** `rules/logical/predicate-pushdown/filter-join-push.rra`

## Metadata

- **ID:** `calcite-filter-join`
- **Version:** "1.0.0"
...
```yaml
  ...
  - type: "predicate"
    condition: "references_only(?pred, ?left) || references_only(?pred, ?right)"
    description: "Predicate must reference columns from only one side"
```
