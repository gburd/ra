...
|---|---|---|---|
| Index-stats double-free | **ACTIVE + reliably observable (2026-06-10), fully contained.** `pfree called with invalid pointer 0x…c08100 (header 0x7f7f7f7f7f7f7f7f)` — a PG palloc-level double-free (0x7f = assert-build CLOBBER_FREED_MEMORY). Caught by the planner-hook `catch_unwind` → native fallback → **0 wrong results, 0 crashes** (a full requalify run logs ~58 caught panics with 0 DIFF/0 ERR/0 crash). | **Diagnosis (2026-06-10):** the `BogusFree` is a *late symptom* — it fires in `errfinish`'s cleanup of the next `ereport` after the heap is already corrupted (even the benign "OK" decision log's `errfinish` trips on it). Backtrace: `ra_planner_hook_inner → pgrx ErrorReport::report → do_ereport → BogusFree`. The *early* free (memory freed while Ra still references it) happens during core planning. RULED OUT: `register_feedback`, monitor `maybe_refresh`, `make_scalar_array_op`, and all explicit Ra `pfree`/`list_free` (only the orphaned `timeline_capture` has any). **Single-user mode is NOT a valid repro** — it corrupts on *every* query (a separate pgrx-in-single-user artifact), confounding lldb. The monitor's pg_catalog SPI queries are one trigger (now bypassed — see below), but the bug also fires for other queries. **Next:** ASan-instrumented PG on Linux (macOS blocks lldb attach to a normal backend; single-user lldb is contaminated) to catch the FIRST free. **Mitigation landed:** the planner hook now routes queries straight to the native planner while `monitor::is_refreshing()` (the monitor only introspects pg_catalog views Ra can't plan anyway), removing the monitor's self-inflicted subset of panics. | P1 |
