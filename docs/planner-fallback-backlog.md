...
|---|---|---|---|
| Index-stats double-free | **RELIABLE REPRO FOUND (2026-06-10).** `pfree called with invalid pointer 0x…c08100 (header 0x7f7f7f7f7f7f7f7f)` — a PG palloc-level double-free (the 0x7f is the assert build's CLOBBER_FREED_MEMORY fill). **Contained**: caught by the planner-hook `catch_unwind` → falls back to native PG → results stay correct, but Ra's plan is discarded. | **Repro:** translate a text/`bpchar`/`varchar` `IN`-list so planning proceeds — apply the `make_scalar_array_op` fallback in `expr_translator::build_in_list`/`build_sao_array` (when `OpernameGetOprid` returns InvalidOid, defer to `make_scalar_array_op` + `assign_expr_collations`), then run `SELECT o_orderkey FROM orders WHERE o_orderstatus IN ('O','P')`. Fires 5/5 (and on psql startup queries). **Diagnosis so far:** single-user `lldb` breakpoint on `BogusFree` shows the free surfaces via `pgrx ErrorReport::report → do_ereport` inside `ra_planner_hook_inner` (an error-within-error on an already-corrupted heap — this is the *late* symptom, not the early free). RULED OUT: `register_feedback` (disabling it doesn't help), explicit Ra `pfree`/`list_free` (only the orphaned `timeline_capture` has any). The early free (memory freed while Ra still references it) is in a PG C function called during planning — most likely `stats_bridge` stats/array deconstruction. **Next:** build with ASan (or set a palloc/AllocSetFree watchpoint on the recurring chunk address) to catch the FIRST free. NOTE: the `make_scalar_array_op` IN-list fix is correct but withheld until this is fixed (it reliably routes text-IN queries into the corruption). | P1 |
